...
1. Create `src/actions/ActivatePresetAction.ts`:
   ...
   - If `advancedMode`:
     - Fan out: for each controller, look up `controllerPresets[c.id]`. If no preset mapped for this controller, skip it (do not call setState). Call `WLEDClient.fromHostPort(c.ip).setState({ ps: presetId })` for each.
   - If simple mode:
